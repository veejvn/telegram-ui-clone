...
const FileMessage = ({ msg, isSender }: MessagePros) => {
  ...
  return (
    <div
      className={`w-70 rounded-xl p-2 px-3 max-w-xs flex flex-col shadow-sm select-none self-end ${
        isSender ? "bg-[#dcf8c6] dark:bg-[#4567fc]" : "bg-white dark:bg-[#222e3a]"
      }`}
    >
      {/* File Content */}
      <div className="flex items-center gap-3 max-w-[260px]">
        <button
          ...
          className={`rounded-full p-3 shrink-0 ${
            isSender ? "bg-[#79c071] dark:bg-white" : "bg-[#74b4ec]"
          }`}
