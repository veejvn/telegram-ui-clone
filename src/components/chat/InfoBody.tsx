...
export default function InfoBody({ user }: { user: sdk.User }) {
  ...
  React.useEffect(() => {
    const fetchCustomData = async () => {
      ...
      try {
        // Custom name
        const nameEvent = client.getAccountData(CUSTOM_NAME_EVENT as any);
        const nameData = nameEvent?.getContent<Record<string, string>>();
        const name = nameData?.[user.userId];
        if (name) setCustomName(name);

        // Custom avatar
        const avatarEvent = client.getAccountData(CUSTOM_AVATAR_EVENT as any);
        const avatarData = avatarEvent?.getContent<Record<string, string>>();
        const mxc = avatarData?.[user.userId];
        if (mxc) {
          const url = client.mxcUrlToHttp(mxc, 96, 96, "crop");
          ...
          >
            <Avatar className="w-24 h-24 mb-2">
              {customAvatarUrl || avatarUrl ? (
                <Image
                  src={(customAvatarUrl || avatarUrl) as string}
                  alt="avatar"
                  width={96}
                  height={96}
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                <div className="bg-transparent mt-2 rounded-none">
                  ...
                  >
                    ...
                    {mediaMessages.length > 0 && (
                      <button
                        className={`flex flex-col items-center justify-center min-w-[66px] h-[44px] rounded-[16px] border px-4 py-3 cursor-pointer transition-all ${activeTab === "media"
                    ...
                    {fileMessages.length > 0 && (
                      <button
                        className={`flex flex-col items-center justify-center min-w-[66px] h-[44px] rounded-[16px] border px-4 py-3 cursor-pointer transition-all ${activeTab === "files"
                          ? "bg-[#155dfc] text-white border-[#155dfc]"
                          : "border-[rgba(128,128,128,0.3)] bg-[#8080804D] text-white"
                          }`}
                        style={{
                    ...
                    {/* Voice */}
                    <button
                      className={`flex flex-col items-center justify-center min-w-[66px] h-[44px] rounded-[16px] border px-4 py-3 cursor-pointer transition-all ${activeTab === "voice"
                        ? "bg-[#155dfc] text-white border-[#155dfc]"
                        : "border-[rgba(128,128,128,0.3)] bg-[#8080804D] text-white"
                        }`}
                      style={{
                    ...
                    {linkMessages.length > 0 && (
                      <button
                        className={`flex flex-col items-center justify-center min-w-[66px] h-[44px] rounded-[16px] border px-4 py-3 cursor-pointer transition-all ${activeTab === "link"
                          ? "bg-[#155dfc] text-white border-[#155dfc]"
                          : "border-[rgba(128,128,128,0.3)] bg-[#8080804D] text-white"
                          }`}
                        style={{
                    ...
                    {/* Groups */}
                    <button
                      className={`flex flex-col items-center justify-center min-w-[66px] h-[44px] rounded-[16px] border px-4 py-3 cursor-pointer transition-all ${activeTab === "groups"
                        ? "bg-[#155dfc] text-white border-[#155dfc]"
                        : "border-[rgba(128,128,128,0.3)] bg-[#8080804D] text-white"
                        }`}
                      style={{
