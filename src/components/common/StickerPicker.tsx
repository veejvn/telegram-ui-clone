...
const STICKER_PACKS = [
  { name: "PEPEtop", path: "/chat/stickers/PEPEtop", count: 117 },
  { name: "Cats", path: "/chat/stickers/Cats", count: 32 },
];

type StickerPickerProps = {
  isOpen: boolean;
  onClose: () => void;
  onStickerSelect: (url: any) => void;
                ...
                <div className="grid grid-cols-5 gap-2 max-h-72 overflow-y-auto">
                  {currentStickers &&
                    Array.from({ length: currentStickers.count }).map(
                      (_, idx) => {
                        const src = `${currentStickers.path}/${idx + 1}.webp`;
                        return (
                          <button
                            key={idx}
